...
#### 리덕스 더 편하게 사용하기
1) redux-actions: 액션생성함수와 리듀서를 createAction, handleActions라는 함수로 더 짧게 작성할 수 있게 해준다. createAction으로 액션을 만들면 액션에 필요한 추가 데이터는 payload라는 이름으로 사용한다.

2) immer: 리듀서에서 상태를 업데이트할 때 불변성을 지키기 위해 spread연산자(...)와 배열 내장 함수를 사용한다. 그러나 모듈의 상태가 복잡해지거나 객체로 이루어진 배열을 다룰 경우엔, immer를 사용하면 더 편하게 상태를 관리할 수 있다.
